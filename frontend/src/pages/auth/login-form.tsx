...
import { useLoginMutation, useLoginWithGoogleMutation } from "@/redux/API/auth-api-slice";
...
export default function Login() {
  ...
  const [loginWithClerk] = useLoginWithGoogleMutation();
  ...
  const loginWithGoogle = async () => {
    try {
      ...
      if (token) {
        const res = await loginWithClerk(token).unwrap();
      ...
      if (typeof err === "object" && err !== null && "data" in err) {
            const errorData = (err as { data?: { message?: string } }).data;
            toast.error(errorData?.message || "Failed to sync login with backend");
          } else {
            toast.error("Failed to sync login with backend");
          }
  ...
  useEffect(() => {
    if (isLoaded && isSignedIn && !userInfo && !isLoading) {
      loginWithGoogle();
    }
